...
**実証研究の専門家のための agent-skills ディストリビューション。** マーケティング用のリストではありません —— 本リポジトリには **1,099 個のスキルを取り込み（vendored）、カタログ化**したうえで、**数値ベンチマーク・評価ハーネス・セキュリティ監査・CI** で品質を固定し、さらに広域エコシステムの **119 リポジトリにまたがる 23,000+ スキル**を精選したマップを重ねています。
...
| 内容 | 数 | 一次情報源 |
|---|---:|---|
| **本リポジトリに取り込み済み**でカタログ化されたスキル | **1,099** | [`catalog/skills.json`](catalog/skills.json) |
...
| **来歴とライセンス** | 取り込まれていないソース、ライセンスリスク、1,099 個すべてのカタログ化スキルにわたる衛生のドリフト | [`docs/LICENSE_AUDIT.md`](docs/LICENSE_AUDIT.md) · [`docs/SKILL_QUALITY.md`](docs/SKILL_QUALITY.md) |
